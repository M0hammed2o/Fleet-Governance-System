...
Two consecutive stable 41-step `npm run phase17a:gate` runs passed at executable candidate `4325b87` (2026-08-19), after fixing a Windows Docker-executable-path resolution defect in `verify-clean-migrations.mjs` and `verify-local-backup-restore.ts` (Node's `child_process` does not reliably inherit Docker Desktop's CLI directory on `PATH` via `cmd.exe`). Each gate reported all 32 migrations replayed/restored clean, 96 Vitest files / 915 tests plus 5/48 facial-contract and 2/20 log-redaction focused runs, a 119/119-page production build, 32/32 Playwright tests, 7/21 mobile tests, a zero-vulnerability dependency audit, and an unchanged Android debug APK (SHA-256 `D6DBD981ABF1D6E53C95C7E60859026B6F7BAC0EBDE0640255F0759B0706546B`, identical to Phase 17B). Gate A ran 33.3 minutes; Gate B ran 31.4 minutes. No online Android emulator/device was available, so the physical-device install/launch smoke step remained correctly skipped, consistent with the existing documented AVD limitation.

Two further consecutive stable 41-step gates passed at executable candidate `aa17fb8` (2026-08-20), after fixing a Docker builder-stage defect where `apps/mobile/node_modules` (holding a non-hoisted `@capacitor/cli`) was silently dropped between build stages, breaking a genuinely clean build such as Render's. A first gate attempt at this candidate failed Playwright with 18 failures; investigation traced this to an unrelated project (`GenBridge\Website`) already holding port 3000, which Playwright's `reuseExistingServer` config silently reused instead of this app's own dev server — not a regression here. That attempt was excluded and the consecutive count restarted, consistent with this repository's existing practice for transient/environmental gate failures. Both final gates reported identical results to the `4325b87` gates above (same migration count, test counts, Playwright count, mobile counts, dependency audit, and Android APK hash). Gate A ran 33.5 minutes; Gate B ran 32.2 minutes.
