...
Pilot imports remain validation-only and cannot mutate the database or send invitations. The UAT catalogue and verification tools are local evidence, not deployment approval. Provider state remains synthetic/mock/no-op/disabled and source-labelled; no biometric template is generated.

Candidate verification composes existing independent commands in `scripts/release-candidate-check.mjs`. It writes only exit codes, durations and commit identity beneath ignored `.data`; command output is not copied into the evidence file. The expected non-zero production-readiness result is asserted as a pass condition while providers and approvals are absent. No Phase 14 schema or migration was added because fixture/UAT state does not belong in production history.

# Phase 15A tracker, UAT and staging architecture

`SyntheticTrackerSimulator` implements the neutral contract with a deterministic clock, explicit MOCK/SIMULATOR provenance, bounded pages, polling checkpoints, signed local webhooks, replay rejection, revocation, outages/recovery and data-quality scenarios. It refuses production and has no HTTP client. `runTrackerConformanceSuite()` is an adapter-neutral activation gate; provider-specific tests can be added only from official documents.
