...
const RECURRING_CYCLE_TEST_TIMEOUT = 120_000;

describe("Phase 10 (P10L): recurring billing cycle", () => {
  beforeAll(async () => {
    const { session } = await makeSession("Platform Administrator", [
      ["platformBilling", "CONFIGURE"],
    ]);
    await updatePlatformBillingSettings(session, {
      vatEnabled: false,
  ...
  it(
    "generates exactly one invoice per active tenant for the reference month",
    async () => {
      ...
      expect(invoiceB?.subtotalMinorUnits).toBeGreaterThan(
        invoiceA?.subtotalMinorUnits ?? 0,
      ); // B has more vehicles
    },
  ...
  it(
    "running the exact same cycle twice never duplicates an invoice or a charge",
    async () => {
      ...
      const snapshots = await prisma.billableVehicleSnapshot.findMany({
        where: { tenantId: tenant.id },
      });
  ...
  it(
    "does not bill the platform tenant itself",
    async () => {
      ...
      await runRecurringBillingCycle(reference);
      const platformInvoices = await prisma.invoice.findMany({
  ...
  it(
    "marks an overdue invoice and, once the grace period has genuinely elapsed, automatically suspends",
    async () => {
      const tenant = await createTenant();
      await createVehicle(tenant.id);
      ...
      await prisma.invoice.update({
        where: { id: invoice.id },
        data: { dueDate: new Date(Date.now() - 60 * 24 * 60 * 60 * 1000) },
      });

      const runNow = await runRecurringBillingCycle(
        new Date(Date.UTC(2027, 4, 1)),
      ); // a distinct new period so no duplicate-invoice interference
