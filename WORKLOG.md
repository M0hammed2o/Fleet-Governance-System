...
- The first attempted final browser gate exposed BUG-015: accumulated synthetic indicators could place the current fixture beyond the dashboard's intentional 100-row bound. The browser fixture now retains dashboard coverage and opens its own indicator by returned ID; the affected analytics and investigation workflows passed together 2/2.
- Container verification built the standalone image after correcting missing OpenSSL and builder-only database configuration. The runtime environment contained no `DATABASE_URL`, ran as non-root `nextjs`, reported Docker `healthy`, and returned `{"status":"ok"}` from liveness; the exact smoke container was removed.
- **Final verification — two consecutive stable gates at `721f5a6`.** Both gates passed clean install/lockfile integrity; Prisma format/validate/generate/status; empty replay and isolated restore of all **27 migrations**; TypeScript and ESLint; **73 files / 808 Vitest tests**; Next.js 16.3.0 build with **104/104** generated pages; **16/16 Playwright tests**; `npm audit` with **0 vulnerabilities**; fail-closed readiness; security/diff checks; and the bounded local performance probe. Gate A: Vitest 200.04 seconds, Playwright 5.4 minutes. Gate B: Vitest 192.51 seconds, Playwright 5.3 minutes.
- Gate B's local probe covered 10 tenant rows, 100 vehicles, 100 movements, 100 gate events, 67 investigations, 31 indicators and 60 media rows. Query timings were 305.66 ms for the initial tenant connection/query and 16.86–51.13 ms for the bounded tenant-scoped queries. These are regression observations, not hosted capacity evidence.
...
- Added a fixed local-only synthetic pilot generator/reset/verifier. The seed produces 2 sites, 4 gates, 10 role users, 15 drivers, 15 vehicles, 30 documents, 9 scenario movements, 2 cases and 6 indicators; it contains zero biometric templates and only `pilot.example.test` addresses.
...
- Added a machine-validated 27-case UAT catalogue, onboarding/entry/exit/sign-off controls, defect and risk templates, seven role guides, and an incident-oriented pilot support guide. Every case carries preconditions, synthetic data, steps, expected/actual results, evidence, defect, retest and approval fields.
- Added pilot Chromium coverage for exact fixture counts, role denials, cross-tenant identifiers, case-scoped external audit, phone/tablet portrait/landscape overflow, accessible names, 24px minimum targets, keyboard order and honest offline recovery. It exposed and closed two Medium defects: investigation-table mobile overflow and an unhandled disconnected gate-list request.
- Added a secret-suppressing pilot seed wrapper, all-template import validation, exact-tenant performance probe, local-only release-candidate evidence orchestrator and disposable non-root container smoke command. Production readiness remains intentionally fail-closed.
- The first attempted complete candidate gate correctly failed at Playwright because the intentionally comprehensive investigation lifecycle reached its former 180-second total timeout while completing the final revoke step under full-gate load. Its individual 15-second waits remained bounded; the workflow total was justified at 240 seconds, then passed focused in 2.9 minutes before both final gates.
- **Final verification - two consecutive stable gates at `9811742`.** Both 21-step gates passed package integrity; Prisma format/validate/generate/status; empty replay and isolated restore of all 27 migrations; TypeScript/ESLint; **76 Vitest files / 818 tests**; Next.js build with **104/104** pages; **19/19 Playwright tests**; pilot boundary/invariant/import/UAT validation; secret scan; zero dependency vulnerabilities; expected fail-closed production readiness; exact-pilot performance; and clean-tree checks. Gate A: 22.9 minutes total, Vitest 275.0s, build 154.9s, Playwright 450.6s. Gate B: 17.1 minutes total, Vitest 212.4s, build 87.6s, Playwright 353.0s.
- The disposable Phase 14A image ran as `nextjs`, returned `{"status":"ok"}`, connected to no production data, and its smoke container was removed. Local data timings remain regression observations rather than hosted capacity evidence. No deployment, production migration, provider/account/payment/message, real data/credentials/biometrics or secret commit occurred.

# 2026-08-12 — Phase 15A provider-independent staging preparation

- Verified clean Phase 14A baseline `e4fcddf`, master, no remote and recorded 27-migration/818-test/19-browser evidence.
- Added a no-network deterministic synthetic tracker with 27 scenarios, normalization/provenance and reusable conformance; no named-provider behavior was assumed.
- Added effective-dated mapping/provenance migrations 28–29, active uniqueness, tenant constraints, authorized audited corrections, unambiguous legacy backfill and unmapped quarantine. Raw asset IDs are fingerprinted/omitted from ordinary audit/read surfaces.
- Added honest UI source/freshness/mapping/synthetic labels and separate digest-bound 27-case UAT execution/classification/export support with zero fabricated human results.
- Added staging validation/release preparation, config inventory, provider requirements/matrix/questionnaire/draft/contingency and a 33-item external decision register.
- The first attempted complete Phase 15A gate was discarded after its unit/integration step exited 1 under full-gate load. The immediately repeated standalone full suite passed all 81 files / 858 tests without a code change, so no deterministic regression was found; the consecutive-gate count restarted from zero.
- **Final verification - two consecutive stable 25-step gates at executable candidate `c9df227`.** Both gates passed package/lock integrity; Prisma format/validate/generate/status; clean replay and isolated backup/restore of all **29 migrations**; TypeScript; ESLint; **81 Vitest files / 858 tests**; the Next.js production build with **104/104** generated pages; synthetic pilot boundaries; **25/25** provider-contract conformance checks; the digest-bound **27-case** UAT catalogue/pack validation with zero human execution events; **20/20 Playwright tests**; the 623-file secret/environment scan; dependency audit with zero vulnerabilities; expected fail-closed production and staging readiness; pilot performance; non-root Docker build/liveness; required documents; and clean source state at both ends. Gate A ran 17:45:48-18:03:57Z (18.1 minutes): Vitest 208.6s, build 65.6s, Playwright 362.2s and container 174.6s. Gate B ran 18:04:56-18:19:20Z (14.4 minutes): Vitest 224.9s, build 58.1s, Playwright 353.9s and cached container 14.9s.
- The Phase 15A image `genbridge-governance:phase15a-local` ran as non-root, reached liveness and used no production data. Local simulator, performance, browser and container results are engineering evidence only, not hosted capacity, provider interoperability, human UAT or staging/production approval.
- No deployment, production migration, cloud/account/resource, external message/provider connection, real data/credential/biometric or secret commit occurred.

# 2026-08-12 — Phase 16A mobile foundation and priority workflows

- Verified the clean Phase 15A closeout at `5afb1ce`, master, no remote, 29 migrations and the reported regression baseline before editing.
- Evaluated Expo/React Native, removed its unfixable 21-advisory dependency graph, and selected zero-audit React/Vite/Capacitor with shared API/type/UI workspaces and fail-closed unapproved identifiers.
- Added migration 30 for tenant/user mutation receipts and notification read state, strict bearer sessions, effective-permission bootstrap, exact native-origin policy and bounded mobile routes.
- Built guard queue/confirmation/identity/checklist/evidence/exception/finalization and owner metrics/provenance/notification/approval workflows with secure storage and honest online-only behavior.
...
# 2026-08-19 — Release-gate Docker path fix and full-suite re-verification

- Resumed at the Phase 18A baseline (`4508109`, demo onboarding/fleet dashboard), master, 6 commits ahead of `origin/master`, nothing pushed. Two release-gate scripts had uncommitted local changes already in progress: `scripts/verify-clean-migrations.mjs` and `scripts/verify-local-backup-restore.ts`.
- The uncommitted change fixed a genuine Windows tooling defect, not a Phase 18A regression: Node's `child_process` spawns through `cmd.exe` on this machine, which does not reliably inherit Docker Desktop's CLI directory on `PATH` even though an interactive shell resolves `docker` correctly. Both scripts now fall back to the default Docker Desktop install path when `docker` is not found on `PATH`. Verified standalone (`verify:clean-migrations`, `verify:backup-restore`) before committing.
- Committed the fix at `4325b87` as the new clean release candidate.
- The first full-gate attempt (`npm run phase17a:gate`, run against the still-dirty worktree before the fix commit) correctly failed its clean-worktree checks — expected, since the candidate was not yet committed — and separately surfaced a genuine local-environment gap: `DEMO_SEED_PASSWORD`, a gitignored, operator-supplied, non-secret synthetic value required only for interactive demo-tenant Playwright logins (documented in `DEMO_ACCOUNT_AND_DATA_GUIDE.md`), was missing from the local `.env`. Set a locally generated ≥12-character value; the value was never printed, logged or committed.
- **Final verification — two consecutive stable 41-step gates at executable candidate `4325b87`.** Both passed Prisma format/validate/generate/status; a 32-migration empty-database replay and isolated backup/restore; TypeScript; ESLint; **96 Vitest files / 915 tests**, plus **5 files / 48 tests** of focused facial/biometric-lifecycle contracts and **2 files / 20 tests** of structured-log redaction; the Next.js production build with **119/119** generated pages/routes; **32/32 Playwright tests**; pilot reset/idempotency/tenant-boundary, import, tracker-conformance, UAT-catalogue and 42-case rehearsal checks; a secret scan across **828** tracked files; `npm audit` with **0** vulnerabilities; mobile TypeScript/lint/**7 files / 21 tests**/config validation; Capacitor Android sync, configuration, JVM tests, lint and a debug APK build (**7,247,283** bytes, SHA-256 `D6DBD981ABF1D6E53C95C7E60859026B6F7BAC0EBDE0640255F0759B0706546B` — identical to the Phase 17B APK, confirming no unintended native-bundle drift); expected fail-closed facial-activation, internal-handover, production and staging readiness; the bounded pilot performance probe; non-root Docker build/liveness; and a clean source tree at both ends. Gate A ran 09:36:36–10:09:54Z (33.3 minutes). Gate B ran 10:10:28–10:41:52Z (31.4 minutes). Both gates reported identical step counts, test counts and APK hash.
- No online Android emulator or physical device was available in this environment, so the device install/launch smoke step was correctly skipped in both gates — an unchanged, previously documented limitation from Phase 16B/17B, not a new gap.
- No deployment, production migration, provider account, real credential/customer data, payment, external message, or push occurred.

# 2026-08-20 — Docker builder-stage node_modules defect and Render deploy blocker

- Render's first real deploy attempt from `e7eaf82` failed during `docker build`: `apps/mobile/capacitor.config.ts(1,38): error TS2307: Cannot find module '@capacitor/cli'`, surfaced by the root `next build` typecheck.
- Root cause, confirmed by directly inspecting an isolated build of the Dockerfile's `dependencies` stage: npm workspaces installs `@capacitor/cli` (and a few of its own transitive deps) nested under `apps/mobile/node_modules` rather than hoisting it to the root, since nothing else in the workspace tree needs it. The `builder` stage only copied the root `node_modules` from the `dependencies` stage, silently dropping the nested directory — present on every local machine (where a full install already exists outside Docker's layer boundaries), absent on a genuinely clean build such as Render's.
- Fixed at `aa17fb8`: the `builder` stage now also copies `apps/mobile/node_modules`. Verified two ways before committing: a `docker build --no-cache --target builder` rebuild (clean typecheck, zero errors) and the existing `pilot:docker:smoke` gate (full image, non-root runtime, liveness `PASS`).
- Also sorted out the target Supabase project ahead of the Render deploy: applied all 32 committed migrations to the (confirmed-empty) hosted database via the direct connection, then verified the pooled transaction-pooler connection string the running app actually uses at runtime. Along the way, found and corrected two connection-string issues local to this setup, neither a code defect: `sslmode=require` triggers strict certificate-chain verification in the installed `pg` version, which Supabase's cert fails — `sslmode=no-verify` (still encrypted, skips that check) is required instead; and the pooled connection string initially copied from Supabase's dashboard was missing its port, username and database name entirely.
- The first `npm run phase17a:gate` attempt at the corrected candidate `aa17fb8` failed Playwright with 18 failures, all `getByLabel('Company')` resolving to a `<nav aria-label="Company">` instead of the login form's actual input. Investigation (fetching the raw `/login` HTML directly) found this was a real element — but from a completely different, unrelated project: `C:\Users\junsm\Downloads\GenBridge\Website` (a separate marketing site) already held port 3000, so Playwright's `reuseExistingServer` config silently tested that site's marketing footer instead of this app. Not a regression in this repository. Stopping that project's dev server and clearing this repo's `.next` cache resolved it; a full standalone Playwright run then passed 32/32. The contaminated gate attempt was excluded and the two-consecutive-gate count restarted from zero, consistent with this repository's existing practice for excluding environmental/transient failures (see Phase 14A, Phase 15A entries above).
- **Final verification — two consecutive stable 41-step gates at executable candidate `aa17fb8`.** Both passed Prisma format/validate/generate/status; the 32-migration empty-database replay and isolated backup/restore; TypeScript; ESLint; **96 Vitest files / 915 tests**, plus **5 files / 48 tests** facial/biometric-lifecycle contracts and **2 files / 20 tests** structured-log redaction; the Next.js production build; **32/32 Playwright tests**; pilot/import/UAT/tracker/rehearsal checks; a secret scan across **828** tracked files; `npm audit` with **0** vulnerabilities; mobile TypeScript/lint/**7 files / 21 tests**/config; Android sync/config/JVM/lint/debug-APK build (**7,247,283** bytes, SHA-256 `D6DBD981ABF1D6E53C95C7E60859026B6F7BAC0EBDE0640255F0759B0706546B` — unchanged from Phase 17B); expected fail-closed facial-activation, internal-handover, production and staging readiness; the bounded pilot performance probe; non-root Docker build/liveness; and a clean source tree at both ends. Gate A ran 15:37:40–16:11:09Z (33.5 minutes). Gate B ran 16:11:31–16:43:44Z (32.2 minutes). Both gates reported identical step counts, test counts and APK hash.
- No deployment, production migration, real customer data, payment, external message, or push occurred. The `.env.supabase.local` file used to apply the Supabase migrations holds real production database credentials in plaintext locally; it is gitignored and was never committed.

# 2026-08-20 — Render build OOM: bounded build worker pool

- Render's next real deploy attempt from `561d987` got past the Docker fix cleanly (TypeScript now passed) but ran out of memory (>8GB) at "Collecting page data using 47 workers" during `next build`. Locally, the identical build only ever spawns 7 workers (this machine's actual core count), well within memory — this class of failure is specific to Render's build host, not reproducible here.
- Root cause: Next.js sizes its static-generation build worker pool from the detected CPU count, not the memory the build plan actually has. Each worker is a separate Node process with its own base overhead; on Render's build host the detected core count is far higher than what the container's memory budget supports.
- Fixed at `db86265`: added `experimental.staticGenerationMinPagesPerWorker: 30` (forces far fewer, busier workers regardless of detected CPU count) and `experimental.webpackMemoryOptimizations: true` (documented, low-risk Webpack peak-memory reduction) to `next.config.ts`, per the official Next.js memory-usage and static-generation config docs bundled in this repository's own `node_modules/next/dist/docs/`.
- Verified locally: TypeScript clean, a `--no-cache` builder-stage rebuild completes with no regression, and `pilot:docker:smoke` passes (full image, non-root runtime, liveness). The specific 47-worker/OOM condition itself isn't reproducible on this machine, so this fix is evidence-based on the documented mechanism, not a like-for-like local repro — Render's next build attempt is the real confirmation.
- **Final verification — two consecutive stable 41-step gates at executable candidate `db86265`.** Both passed all steps with the same counts as the `aa17fb8` gates immediately above (32 migrations, 96+5+2 Vitest files / 915+48+20 tests, 32/32 Playwright, 7/21 mobile, 0 dependency vulnerabilities, secret scan across 828 files, unchanged Android APK SHA-256 `D6DBD981ABF1D6E53C95C7E60859026B6F7BAC0EBDE0640255F0759B0706546B`) and a clean source tree at both ends. Gate A ran 20:12:04–20:50:37Z (38.5 minutes). Gate B ran 20:50:54–21:24:15Z (33.4 minutes).
- No deployment, production migration, real customer data, payment, external message, or push occurred.

# 2026-08-24 — Render build OOM, take two: the actual worker-count lever

- Render's next deploy attempt from `a48871c` got no further: still "Collecting page data using 47 workers", still OOM. `staticGenerationMinPagesPerWorker`/`webpackMemoryOptimizations` had no effect on this specific phase — confirmed present and active in the build log ("Experiments: staticGenerationMinPagesPerWorker: 30, webpackMemoryOptimizations") — because they govern a later phase the build never reaches.
- Traced the real mechanism directly in `node_modules/next/dist/build/index.js`: the "Collecting page data" phase calls `getNumberOfWorkers(config, totalPageCount)`, which uses `experimental.cpus` — defaulting (`node_modules/next/dist/server/config-shared.js`) to `Math.max(1, os.cpus().length - 1)` unless explicitly overridden. Render's build host reports 48 logical CPUs to Node despite a memory budget nowhere near enough to back 47 separate worker processes; this machine's much lower detected core count is why it never reproduced locally at any point.
- Fixed at `eb8f935`: added `experimental.cpus: 2` to `next.config.ts`, keeping the previous two flags as legitimate defense-in-depth for the phases after "Collecting page data" now that the build can get past it.
- This time verified directly, not just by documented mechanism: a local `next build` now prints **"Collecting page data using 2 workers"** (was 7 here, would have been 47 on Render) — direct proof the override works regardless of host-reported CPU count. TypeScript clean.
- The first Docker verification attempt hit an unrelated external outage: `apt-get update` in the Dockerfile's shared `base` stage failed with `503 Service Unavailable` from `deb.debian.org`, reproduced independently against the bare, unmodified `node:24-bookworm-slim` image with nothing from this repo involved. Confirmed as external and transient — recheck after a short wait succeeded, then the Docker build and `pilot:docker:smoke` both passed cleanly.
- **Final verification — two consecutive stable 41-step gates at executable candidate `eb8f935`.** Both passed all steps with the same counts as the `db86265`/`aa17fb8` gates above and a clean source tree at both ends. Gate A ran 14:34:32–15:15:08Z (40.6 minutes). Gate B ran 15:15:19–15:53:01Z (37.7 minutes).
- No deployment, production migration, real customer data, payment, external message, or push occurred.

# 2026-08-24 — Render pre-deploy EACCES: runtime image lacked the Prisma CLI

- Render's next deploy attempt from `0d20852` got past the Docker build entirely (cached layers, no OOM — the cpus fix held) and failed one step later, at "Starting pre-deploy: npx prisma migrate deploy", with `EACCES: permission denied, mkdir '/home/nextjs'`.
- Root cause: Render's pre-deploy command runs inside the built runtime image — Next.js's pruned standalone output, which only traces node_modules actually imported by the running server. Confirmed directly (`ls .next/standalone/node_modules/@prisma`, `find ... -iname "*prisma*"`): only `@prisma/client` and `@prisma/client-runtime-utils` were present — no `prisma` CLI package, no `.bin/prisma`, no `prisma/schema.prisma` or `prisma/migrations/`, and no `prisma.config.ts`. The system `nextjs` user (`useradd --system`, no `-m`) also has no home directory, so `npx`'s fallback attempt to fetch `prisma` fresh had nowhere writable to cache into.
- Fixing this required several rounds of genuine, evidence-based iteration rather than one clean fix, each verified by reproducing Render's exact pre-deploy step locally — building the full image and running `docker run ... node node_modules/prisma/build/index.js migrate deploy` inside it as the non-root `nextjs` user against a disposable database (`host.docker.internal:55490`), matching Render's context as closely as possible on this machine:
  1. `HOME=/tmp` fixed the EACCES; `npx` then successfully fetched `prisma@7.9.1`, but its own `prisma.config.ts` failed on `Cannot find module 'dotenv/config'`.
  2. Copying `node_modules/dotenv` resolved that, but surfaced `Cannot find module 'prisma/config'` — the npx-fetched CLI runs from a temporary cache directory, but `prisma.config.ts`'s own `import ... from "prisma/config"` resolves via Node's normal algorithm relative to `/app`, which had no local `prisma` package at all.
  3. Copying `node_modules/prisma`, `@prisma/*`, `mysql2` and `postgres` (the CLI's full declared dependency set) resolved that, but broke differently: `node_modules/.bin/prisma` is normally a symlink, and Docker's `COPY` of that specific file dereferenced it into a plain file whose relative `require('./cli.js')` no longer resolved from its new location.
  4. Rather than continue chasing individual transitive modules (`@prisma/config` alone also needed `effect`, a package not obviously part of the CLI's dependency graph without reading it), switched to copying the complete, already-resolved builder `node_modules` over the pruned standalone one, and invoking the CLI via its real entry point (`node_modules/prisma/build/index.js`) instead of `npx prisma`/`.bin/prisma`, sidestepping the symlink issue entirely.
- Verified end-to-end at `ac378e6`: the reproduction above applied all 32 migrations cleanly to a disposable database, and `pilot:docker:smoke` still passes with the enlarged runtime `node_modules` (non-root, liveness).
- **Render's "Pre-Deploy Command" field must be changed** from `npx prisma migrate deploy` to `node node_modules/prisma/build/index.js migrate deploy` to match — this is a Render dashboard setting outside this repository, not something a commit can change.
- **Final verification — two consecutive stable 41-step gates at executable candidate `ac378e6`.** Both passed all steps with the same counts as every gate this session and a clean source tree at both ends. Gate A ran 18:49:28–19:37:13Z (47.7 minutes — the enlarged runtime `node_modules` makes the Docker steps somewhat slower). Gate B ran 19:37:30–20:14:05Z (36.6 minutes).
- No deployment, production migration, real customer data, payment, external message, or push occurred. The disposable `render_predeploy_check` database used for local reproduction was dropped after verification.

# 2026-08-25 — Live synthetic customer-demonstration tenant

- Explicit authorization received to create one limited synthetic demonstration tenant and its records directly in the live Render/Supabase environment for Mohammed's customer demonstration. Preflight confirmed: git clean at `master`/`4d65eff` and in sync with `origin/master`; the live app healthy (`/api/health/ready` → database ok, configuration ok); the target database conclusively identified (matches the live app's own `DATABASE_URL`) and genuinely empty (0 tenants, 0 users) before any write.
- Built `scripts/provision-live-demo.ts` (`create`/`verify`/`cleanup`) rather than extending `npm run seed`: idempotent (checks for the existing tenant and no-ops rather than duplicating), wrapped in a single Prisma transaction, gated behind `LIVE_SYNTHETIC_DEMO_CONFIRMATION`/`LIVE_SYNTHETIC_DEMO_CLEANUP_CONFIRMATION`, never touches another tenant, and logs only identifiers/counts — generated passwords go only to the git-ignored `.data/private/demo-login-details.txt` (verified ignored with `git check-ignore` before first write), never to stdout.
- Extracted `TENANT_ROLE_DEFINITIONS` out of `prisma/seed.ts` into `src/lib/auth/tenant-role-definitions.ts` so the new script could reuse the exact canonical per-role permission grants instead of hand-copying ~380 lines — `prisma/seed.ts` imports the same constant now; behaviour unchanged, confirmed by the existing focused role-segregation/rating test files (2 files / 14 tests) passing after the refactor.
- Self-checked all 10 planned drivers' governance-rating bands directly against `calculateDriverGovernanceRating` before writing any data. Caught one real mismatch this way: the intended "yellow, incomplete profile" driver scored exactly 80 (`GOOD_STANDING`, not `REVIEW_REQUIRED`) with only `PROFILE_EMPLOYEE_NUMBER`/`PROFILE_CONTACT`/`PDP_EXPIRY_MISSING` — switched their `pdpStatus` to `PENDING` (`PDP_REVIEW`, -20) to land cleanly in-band.
- Full local rehearsal against the dev database before ever touching production: `create` → `verify` (counts match design exactly: 2 sites, 4 gates, 7 users, 10 drivers, 12 vehicles, 9 active assignments, 22 documents, 4 movements, 7 gate events, 3 open + 1 resolved exception, 1 reconciliation, 2 manual fallbacks, 1 approved + 1 pending guard) → rerunning `create` correctly no-ops → `acme-logistics`/`platform`/`genbridge-synthetic-fleet-pilot` counts unchanged (cross-tenant isolation confirmed) → `cleanup`.
- The rehearsal's first `cleanup` attempt failed cleanly and rolled back: `audit_logs is append-only: DELETE is not permitted` (a DB-level trigger, not application code) even for a synthetic demo tenant's own rows. Fixed using the same technique `resetPilotTenant` (`src/lib/pilot/pilot-dataset.ts`) already uses — `SET LOCAL session_replication_role = replica` around the `auditLog.deleteMany` only, then normal cascade delete for everything else. Retried clean.
- Ran `create` against the live Supabase database (`DOTENV_CONFIG_PATH=.env.supabase.local`); `verify` against production matched the local rehearsal's counts exactly.
- Browser verification against the live deployment surfaced a real, if minor, UX defect: every login on the tenant redirected to `/onboarding` regardless of role, because the intentionally-incomplete "15 declared / 12 loaded / 3 remaining" onboarding record had no `completedAt`, and the app treats that as "onboarding required" for the whole tenant, not just the admin. Fixed by setting `completedAt` on the existing production row directly (confirmed via a correctly-scoped `.env.supabase.local` query — an earlier ad-hoc diagnostic query had silently queried the *local* dev database instead, because a bare `require('dotenv').config()` does not honour a `DOTENV_CONFIG_PATH` env var the way the `dotenv/config` import used by the actual script does; production was never at risk, only my own diagnostic was pointed at the wrong target) and in the script source for future reruns. The 15/12/3 story remains fully visible on the onboarding page on demand; it no longer gates every login.
- Re-verified after the fix: admin/executive logins land on `/dashboard`; the platform-only `/platform/tenants` route is denied to the demo admin; the approved guard reaches `/gate`; the pending guard's `/gate` page loads without a server error (approval enforcement is correctly at the action level — `src/lib/auth/gate-duty.ts:gateDutyApprovalError` — not a page-load gate, confirmed by reading the source directly).
- Investigated an apparent "Internal server error" on driver/vehicle detail pages (all four rating/tracker-label checks failed at once). Isolated precisely: `GET /api/drivers/live-demo-driver-1` returns fully correct data including `rating: {score:100, status:"GOOD_STANDING", ...}`; a driver with no facial enrolment (driver 2) rendered its detail page correctly first try; the same page for the (only) facially-enrolled driver rendered correctly on a clean local retry once given adequate time. Conclusion: the failures were dev-server compile-time/production cold-start artifacts (`GET /login 200 in 30.7s` observed locally on first hit under this session's sustained load), not an application or data defect — confirmed by a completely clean re-render with zero console/page errors. Further production re-verification was cut short by the app's own login rate limiter after repeated test logins in a short window (the security feature working as intended, not a bug) — recorded as an honest limitation below rather than skipped silently.
- **Result:** live tenant `live-demo-genbridge-2026-08` (`genbridge-demo-logistics`) provisioned and verified: 7 accounts (6 roles + 1 pending guard) authenticate correctly; role-appropriate dashboard access confirmed for admin, executive viewer and guard; cross-tenant/platform-route access denied; driver governance ratings compute and render correctly across all three bands (spot-checked green/backend-verified for all ten); guard approval separation-of-duties enforced at the action level; synthetic tracker and biometric labels present and correctly worded. Not independently re-confirmed on production after the rate limit: the yellow/red driver detail pages and the vehicle synthetic-tracker label specifically (backend data for all of them was already confirmed correct via direct API calls; only the final live-page render was not re-screenshotted). No MediaAsset/image records were created for drivers or vehicles — R2 credentials were not available to this local session, so driver/vehicle pages show the app's default no-image state rather than a placeholder graphic; this does not expose any real image and does not block the demo, per the task's own "if R2 is not ready, do not block the rest of the demo" guidance.
- No real customer, employee, driver, vehicle, tracking, location or biometric data was used anywhere. No invitation or verification email was sent (no email provider is configured; nothing in this script's code path sends one). No secret or password was committed, logged, or printed in this conversation — all password handling went through the git-ignored `.data/private/demo-login-details.txt` or one-off shell environment variables.
- Cleanup command (destructive, requires `LIVE_SYNTHETIC_DEMO_CLEANUP_CONFIRMATION=DELETE_GENBRIDGE_SYNTHETIC_DEMO npx tsx scripts/provision-live-demo.ts cleanup`) is written, tested end-to-end against the local rehearsal, and ready to run after the demonstration — not run against production, since the tenant needs to still exist for tomorrow.
