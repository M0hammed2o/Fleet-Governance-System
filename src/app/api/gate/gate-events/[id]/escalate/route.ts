...
import { escalateExceptionToSupervisor } from "@/lib/repositories/gate-event-repository";
...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const gateEvent = await escalateExceptionToSupervisor(session.tenantId, id, session.userId);
