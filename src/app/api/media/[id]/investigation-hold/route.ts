...
import { setInvestigationHold, MediaAssetNotFoundError } from "@/lib/repositories/retention-repository";
...
export async function PATCH(request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const mediaAsset = await setInvestigationHold(session.tenantId, session.userId, id, parsed.data.hold, parsed.data.reason);
