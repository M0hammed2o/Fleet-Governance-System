...
import { markAnalyticsIndicatorReviewed } from "@/lib/repositories/analytics-indicator-repository";
...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    return NextResponse.json({ indicator: await markAnalyticsIndicatorReviewed(session, id, note) });
  } catch (error) {
