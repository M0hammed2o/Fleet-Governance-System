...
import { escalateInvestigationCase } from "@/lib/repositories/investigation-case-repository";
import { escalateCaseSchema } from "@/lib/validation/investigations";
...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const investigationCase = await escalateInvestigationCase(session, id, parsed.data.priority, parsed.data.reason);
