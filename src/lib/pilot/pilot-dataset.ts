...
export async function seedPilotTenant(prisma: PrismaClient): Promise<void> {
  ...
  const cargoMedia = await createSyntheticMedia(prisma, "GATE_EVENT_INSPECTION_ITEM", cargoResult.id, officer.id, "cargo-discrepancy");
  await prisma.gateEventInspectionItem.update({ where: { id: cargoResult.id }, data: { evidenceMediaAssetId: cargoMedia.id } });
  ...
  for (const [scenario, category, departureOdometer, returnOdometer, departureFuel, returnFuel] of [
    ...
    if (category) {
      const linked = await prisma.exception.create({ data: { id: id("exception", scenario), tenantId: PILOT_TENANT_ID, gateEventId: pair.returnId, inspectionResultId: scenario === "cargo-discrepancy" ? cargoResult.id : null, exceptionTypeId: discrepancyType.id, description: `Synthetic ${category.toLowerCase().replaceAll("_", " ")} discrepancy requiring neutral human review.`, severity: "HIGH", requiresSupervisorApproval: true, outcomeAction: "MANUAL_REVIEW", raisedByUserId: officer.id, raisedAt: FIXED_NOW } });
  ...
  const indicatorSpecs = [
    ...
    ["fuel", "Fuel reading difference requires neutral review", "MANUAL", "OPEN", vehicles[4].id, null],
    ["odometer", "Manual and mock odometer readings are inconsistent", "MIXED", "OPEN", vehicles[5].id, null],
  ] as const;
  ...
  await prisma.analyticsCalculationRun.create({ data: { id: id("analytics-run", 1), tenantId: PILOT_TENANT_ID, status: "SUCCEEDED", startedAt: FIXED_NOW, finishedAt: new Date(FIXED_NOW.getTime() + 1_000), rulesEvaluated: 1, indicatorsCreated: 6, dataQuality: "MIXED", resultSummary: { synthetic: true, externalProviderCalls: 0 } } });
